...
router.use(function timeLog(req, res, next) {
	let time = new Date();
	let getTime = (req.getTime = `${time.getFullYear()}-${
		time.getMonth() + 1
...
router.get('/manyhtml', function (req, res) {
	//sendFile只可以传绝对路径
	res.sendFile(
		_mainUrl + '/一个Node服务端测试/public/dist/' + req.query.name + '.html'
	);
	// res.sendFile(_mainUrl+'/手动Vue项目实践/'+req.query.name+'/dist/index.html');
});
...
// 定义 about 页面的路由
router.get('/qwzn', function (req, res) {
	//sendFile只可以传绝对路径
	res.sendFile(_mainUrl + '/js权威指南Demo/' + req.query.name + '.html');
});
...
router.get('/pwa', function (req, res) {
	...
	fs.readFile(
		path.resolve('/_mainUrl/pwa', req.query.name + '.html'),
		'utf-8',
		function (err, data) {
			//读取内容
			if (err) {
...
router.get('/jsWap', function (req, res) {
	...
	fs.readFile(path.resolve('mydist', 'index.html'), 'utf-8', function (
		err,
		data
	) {
		//读取内容
...
router.post('/upload', upload, function (req, res) {
	...
	fs.createReadStream(
		path.resolve(__dirname + '/' + '../public/upload.zip')
	).pipe(
		unzip.Extract({
			path: path.resolve(__dirname + '/' + '../public/' + _url),
		})
	);
